...
def buildTree(val):
    # val is a list of values where None indicates absence of node
    # not val means empty list or first element is -1 means no root
    if not val or val[0] == -1:
        return None
    
    root = TreeNode(val[0])
    nodes = [root]
    i = 1 # index in val list
    n = 0 # index of last node in the list
    # in while loop we check if i < len(val) it means there are more elements to process
    while i < len(val):
        currentNode = nodes[n]
